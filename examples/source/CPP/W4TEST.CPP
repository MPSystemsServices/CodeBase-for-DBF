...
Field4memo  fName, lName, address, age, married,
            amount, comment;
char        gcAlignBytes ;    //byte alignment of first child control in dialog (32-bit only)
...
WORD ConnectToServer( Code4& cb, D4SETTINGS *settings )
{
   ...
   rc = cb.connect( settings->serverId, settings->processId,
...
WORD OpenDataFile( )
{
   ...
   if( dataFile.create( cb, "C4TEST.DBF", field_info, 0 ) == r4success )
   {
...
void PrintRecords( )
{
   ...
   for( int rc = dataFile.top();rc == r4success;rc = dataFile.skip(1L))
   {
      ...
      d4displayStr("------------------------------", 1);
      sprintf(buf, "Name     : %20s", name.ptr() );
      d4displayStr( buf, 1 ) ;
      sprintf(buf, "Address  : %15s", address.str() );
      d4displayStr( buf, 1 ) ;
      sprintf(buf, "Age : %3d   Married : %1s", (int)age, married.str() ) ;
      d4displayStr(buf, 1) ;
      sprintf(buf, "Comment: %s", comment.str() );
      d4displayStr(buf, 1) ;
...
GLOBALHANDLE d4createDialog( void )
{
   ...
   GLOBALHANDLE dlgHandle = d4newDlgTemplate( dlgStyle, 40, 40, 270, 100, UTEXT(""), UTEXT(""), UTEXT("W4Test Settings"), UTEXT(""), 0 ) ;
   if( dlgHandle )
   {
      while( children[i].id )
      {
