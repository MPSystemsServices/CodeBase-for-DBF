...
#include "d4all.h"
   ...
   #if defined( S4FOX ) && !defined( S4UTILS ) && !defined( S4CLIENT )
      static void d4incrementAutoIncrementValue( DATA4 *data )
      {
         if ( data->dataFile->autoIncrementSupported )  // update the auto-increment value...
         {
               ...
               double tempDbl ;  /* LY 2003/07/10 : restored tempDbl */
...
static int d4appendVerifyInputs( DATA4 *data )
{
   ...
   #define d4appendUnlock( a, b, c, d ) ( 0 )
      ...
      #pragma argsused
   #endif
   static void d4appendUnlock( DATA4 *data, Bool5 indexLocked, long serverId, long clientId )
   {
      DATA4FILE *d4file = data->dataFile ;
      ...
      #pragma argsused
   #endif
   static int d4appendLockIndexes( DATA4 *data, Bool5 *indexLocked, const long serverId, const long clientId )
   {
      #ifdef S4SERVER
         DATA4FILE *d4file = data->dataFile ;
      ...
      if ( !(*indexLocked) )
      {
         ...
         {
                        /* AS 07/23/99 --> higher level functions responsible for unlocking if required...
               #ifdef S4SERVER
                  dfile4unlockAppend( d4file, clientId, serverId ) ;
               #else
                  d4unlockAppend( data ) ;
   ...
   static int d4appendUpdateIndexes( DATA4 *data, long serverId, long clientId, Bool5 *indexLocked )
   {
      ...
      if ( hasIndexes )
      {
         int rc = d4appendLockIndexes( data, indexLocked, serverId, clientId ) ;
...
/* not S4OFF_WRITE */
   ...
   int S4FUNCTION d4append( DATA4 *data )
   {
      ...
      if ( data->batchWrite.writeRecsToBuf > 0 && dfile4lockTestAppend( data->dataFile, clientId, serverId ) == 1 )  // buffer writes
         return d4appendBatch( data, serverId, clientId ) ;
      else
         return d4appendRequestFromServer( data, serverId, clientId ) ;
   }
#endif
   ...
   static int d4doAppend( DATA4 *data )
   {
      ...
      {
         d4appendUnlock( data, indexLocked, serverId, clientId ) ;
         return rc ;
      }
   ...
   static int d4appendRegisterTransactionFinish( DATA4 *data, int rc, const int hasTran )
   {
      if ( rc < 0 || rc == r4unique )
      {
         if ( d4transEnabled( data, 1 ) )
      ...
      static Bool5 d4appendFindDeletedRow( DATA4 *data )
      {
         ...
         for ( ;; )
         {
            ...
            if ( rc == 0 && d4deleted( data ) )   // ensure is still deleted (maybe another user appending at same time got record first)
            {
      ...
      if ( rc == 0 )
      {
         ...
         if ( hasTran && rc >= 0 )
         {
            code4tranCommitSingle( data->codeBase ) ;
               ...
               {
                  #ifdef S4SERVER
                     int saveRc = dfile4unlockAppend( data->dataFile, data4clientId( data ), data4serverId( data ) ) ;
                  #else
                     int saveRc = d4unlockAppend( data ) ;
                  #endif
                  #ifndef S4OFF_INDEX
                     if ( saveRc == 0 )
...
short S4FUNCTION d4appendStart( DATA4 *data, short useMemoEntries )
{
   ...
   int S4FUNCTION D4appendDirect( DATA4 S4PTR *data, char S4PTR *buffer )
   {
      ...
      static int d4unappendData( DATA4 *dataIn )
      {
         ...
         data->fileChanged = 1 ;
         // AS Aug 15/01 - Code for 'version number' handling so users can know if the file has changed.
         #if defined(S4WIN32) || !defined(S4STAND_ALONE)  /* LY 2003/06/10 : not defined on UNIX */
            dfile4versionIncrement( data ) ;
         #endif
