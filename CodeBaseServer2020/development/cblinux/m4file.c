         ...
         */
         ...
         if ( avail > (unsigned long)*lenPtr )
         {
            if ( *lenPtr > 0 )
               u4free( *ptrPtr ) ;
            #ifdef S4SERVER
               // AS 02/13/01 - don't consider it critical if running out of memory
               // either memo is corrupt or we just don't have the memory to hold onto the entry
               *ptrPtr = (char *)u4alloc( avail + 2 ) ;  // CS 1999/09/14 allow room for Unicode NULL character
            #else
               *ptrPtr = (char *)u4allocEr( f4memo->file.codeBase, avail + 2 ) ;  // CS 1999/09/14 allow room for Unicode NULL character
            #endif
            if ( *ptrPtr == 0 )
            {
               #ifdef S4SERVER
            ...
            long mType = 1L ;
               ...
               if ( f4memo->data->compressedMemosSupported == 1 )
               {
                  ...
                  if ( ptrLen > effectiveBlockSize )  // if the memo is > 1 memo block, then compress it to save space
                  {
                     ...
                     int rc = c4compress( c4, c4->compressedMemosBuffer, &outLen, ptr, ptrLen, COMPRESS4MEMO_DEFAULT_LEVEL, 1 ) ;
