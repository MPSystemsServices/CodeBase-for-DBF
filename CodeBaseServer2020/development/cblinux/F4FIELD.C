...
int S4FUNCTION date4timeCompare( const S4LONG *dt1, const S4LONG *dt2 )
{
   ...
   #ifdef S4DATA_ALIGN
      memcpy( &date1, &dt1[0], sizeof(S4LONG) ) ;
      memcpy( &date2, &dt2[0], sizeof(S4LONG) ) ;
   #else
      date1 = dt1[0] ;
      date2 = dt2[0] ;
   #endif
   ...
   {
      #ifdef S4DATA_ALIGN
         memcpy( &time1, &dt1[1], sizeof(S4LONG) ) ;
         memcpy( &time2, &dt2[1], sizeof(S4LONG) ) ;
      #else
         time1 = dt1[1] ;
         time2 = dt2[1] ;
      #endif
...
int S4FUNCTION currency4subtract( CURRENCY4 *result, const CURRENCY4 *c1, const CURRENCY4 *c2 )
{
   ...
   // input source is longlong 10000 times greater than actual value (i.e. 4 decimal digits available by dividing input by 10000)
         ...
         *((LONGLONG *)result) = *((LONGLONG *)c1) - *((LONGLONG *)c2) ;
         ...
         #ifdef S4BYTE_SWAP
            long tempC1, tempC2, tempLong, tempResult ;
            unsigned char *ptr, array[8] ;
            int i ;
...
int S4FUNCTION c4atoCurrency( CURRENCY4 *result, const char *str, int strLen )
{
  ...
  */
   ...
   char *ptr ;
      ...
      short int tempSInt ;
   #endif
   /* LY 2003/07/07 : removed S4BYTE_SWAP */
   ...
   if ( numDigits < 10 || ( numDigits == 10 && ( str[0] <= '3' && str[0] >= '0' ) ) )
   {
      ...
      if ( sign == 1 )   /* positive value */
      {
            ...
            #else
               memset( &result->lo[2], 0, sizeof(short int)*2 ) ;
            #endif
         #else
            result->lo[3] = 0 ;
            result->lo[2] = 0 ;
      ...
      else   /* negative value */
      {
            ...
            #else
               memset( &result->lo[2], 0xff, sizeof(short int)*2 ) ;
            #endif
         #else
            result->lo[3] = 0xFFFF ;
            result->lo[2] = 0xFFFF ;
...
char S4PTR *S4FUNCTION f4dateTime( const FIELD4 *field )
{
   ...
   long val ;
   char *fPtr ;
   #ifdef S4DATA_ALIGN  /* LY 2003/07/14 */
      S4LONG tempVal ;
   #endif
   ...
   fPtr = f4ptr( field ) ;
      ...
      #else
         /* LY 2003/07/14 : since val is long, val may be 64-bit */
   ...
   date4assign( codeBase->fieldBuffer, val ) ;
         ...
         #else
            /* LY 2003/07/14 : since val is long, val may be 64-bit */
            memcpy( &tempVal, ((S4LONG*)fPtr) + 1, sizeof(S4LONG) ) ; /* time */
            val = tempVal ;
         #endif
      #endif
   #else
...
void S4FUNCTION f4assignDateTime( FIELD4 *field, const char *dateTime )
{
   ...
   /* input date time of format: "YYYYMMDDHH:MM:SS" */
      ...
      if ( field->type != r4dateTime )
      {
         ...
         tempTime = time4long( dateTime + 8, c4strlen( dateTime ) - 8 ) ;
