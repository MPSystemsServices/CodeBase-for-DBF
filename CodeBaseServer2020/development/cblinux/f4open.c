...
int file4openInternal( FILE4 *file, CODE4 *c4, S4CONST char *name, const int doAlloc, char fileType )
{
   int rc, len ;
...
int S4FUNCTION file4compressOpen( FILE4 *file, CODE4 *c4, S4CONST char *name )
{
      ...
      int rc = file4open( file, c4, name, 0 ) ;  // also checks input paramaters
      if ( rc != 0 )
         return rc ;

      long numOffsets ;
      rc = file4readAll( file, 2 * sizeof( short ), &numOffsets, sizeof( long ) ) ;

      if ( rc == 0 )
      {
         long sizeInfo = sizeof( COMPRESS4DATA ) + (numOffsets - 1) * sizeof( long ) ;  // -1 because sizeof() includes 1 entry
         COMPRESS4DATA *compressInfo = (COMPRESS4DATA *)u4allocFree( c4, sizeInfo ) ;
         if ( compressInfo == 0 )
            rc = -1 ;
         if ( rc == 0 )
            rc = file4readAll( file, 0, compressInfo, sizeInfo ) ;
         if ( rc == 0 )
         {
