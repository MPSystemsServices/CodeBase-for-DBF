...
#include "d4all.h"
      ...
      int dfile4lock( DATA4FILE *data, const long clientId, const long serverId, const long rec, Lock4type lockType )
      {
         ...
         #ifdef E4PARM_LOW
            if ( data == 0 || rec < 1L || clientId == 0 || ( lockType != lock4read && lockType != lock4write && lockType != lock4any )
                 #ifdef S4SERVER
                    || serverId == 0
                 #endif
               )
      ...
      int dfile4lockFile( DATA4FILE *data, const long clientId, const long serverId, DATA4 *d4, Lock4type lockType )
      {
         ...
         #ifdef E4PARM_LOW
            if ( data == 0 || clientId == 0 || ( lockType != lock4read && lockType != lock4write && lockType != lock4any )
                 #ifndef S4CLIENT
                    || serverId == 0
                 #endif
               )
      ...
      int dfile4lockTest( DATA4FILE *dataIn, const long serverId, const long rec, Lock4type lockType )
      {
         ...
         #ifdef E4PARM_LOW
            if ( dataIn == 0 || ( lockType != lock4read && lockType != lock4write && lockType != lock4any ) )
               return error4( 0, e4parm_null, E91102 ) ;
         #endif
