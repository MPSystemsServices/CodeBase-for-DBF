   ...
   S4EXPORT void S4FUNCTION error4out( CODE4 *c4, int errCode1, long errCode2, const char *desc1, const char *desc2, const char *desc3 )
   {
      ...
      if ( c4 != 0 )
         ...
         if (errCode1 != 0)
         {
            ...
            c4strcpy( errorStr + pos, E4_ERROR ) ;
            c4strcat( errorStr + pos, " #: " ) ;
            pos = c4strlen( errorStr ) ;
            c4ltoa45( error4number2( errCode2 ), (char *)errorStr + pos, 6 ) ;
            pos += 6 ;
         ...
         while ( (ptr != (char *) 0) && (descNumber <= 3 ) )
         {
            ...
            if ( s4fileName != 0 )
               ...
               {
                  c4strcpy( errorStr + pos, s4fileName ) ;
...
static void error4storeDescription(CODE4 *c4, const char *s1, const char *s2, const char *s3)
{
   ...
   if (s1 || s2 || s3)
   {
      ...
      c4->lastErrorDescription = (char *)u4allocFree(c4, descriptionLength + 6);

      if (s1)
      {
         strcpy(c4->lastErrorDescription, s1);
      }
      if (s2)
      {
         if (strlen(c4->lastErrorDescription) > 0)
            strcat(c4->lastErrorDescription, "\r\n");
         strcat(c4->lastErrorDescription, s2);
      }
      if (s3)
      {
         if (strlen(c4->lastErrorDescription) > 0)
            strcat(c4->lastErrorDescription, "\r\n");
         strcat(c4->lastErrorDescription, s3);
      }
   } // if s1 || s2 || s3
}
...
int S4FUNCTION error4describeExecute( CODE4 *c4, const int errCode1, const long errCode2, const char *s1, const char *s2, const char *s3 )
{
      ...
      if (doCallback)
      {
         c4->errorCallback(c4, errCode1, error4number2(errCode2), s1, s2, s3);  // CS 2002/11/11 translate number 2
      } // if doCallBack
      else
      {
