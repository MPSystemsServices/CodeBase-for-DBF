               ...
               d4unlockData( data, clientId ) ;
                     ...
                     if ( code4unlockAuto( c4 ) == LOCK4DATA )
                        ...
                        else if ( c4->currentClient->sendUnlock == 1 )
                        {
                           ...
                           connect4sendLong( connect, data->clientId ) ;
...
int S4FUNCTION d4unlockData( DATA4 *data, long clientId )
{
   #ifndef S4OFF_MULTI
      #ifdef E4PARM_HIGH
         if ( data == 0 )
...
#endif
/* AS Nov 13/02 - export for dot */
int S4FUNCTION d4unlockFile( DATA4 *data, long clientId )
{
   #ifndef S4OFF_MULTI
      int rc ;
      #ifdef E4VBASIC
...
#endif
/* AS Nov 13/02 - export for dot */
int S4FUNCTION d4unlockRecords( DATA4 *data, long clientId )
{
   #ifndef S4OFF_MULTI
      #ifndef S4CLIENT
         Lock4 *lock ;
