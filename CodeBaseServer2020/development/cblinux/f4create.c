...
//   #endif
   ...
   file->type = fileType ;
      ...
      // #else
      //    file->preprocessed = preprocess4fileHook( &c4->preprocess, file, preprocess4getInit( &c4->preprocess ) ) ;
      // #endif
   #endif
...
int S4FUNCTION file4compressCreate( FILE4 *newFile, FILE4 *sourceFile, S4CONST char *name, short blockSize )
{
      ...
      if ( rc == 0 )
         rc = file4compressInit( newFile, compressInfo, 0 ) ;
