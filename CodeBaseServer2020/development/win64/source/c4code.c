...
// AS Oct 24/01 - Changed client/server interfacing for tag-related functions,
// ensure it is in synch by updating version from 6502 to 6503
#if S4VERSION != 6503003
...
// static void c4unloadDlls( CODE4 *c4 )
// {
//    // AS Jun 19/02 - ability to load components at runtime - unload phase
//    #ifdef S4WIN32
//       if ( c4->zlibDll != 0 )
//       {
//          FreeLibrary( c4->zlibDll ) ;
//          c4->zlibDll = 0 ;
//       }
//    #endif
// }
...
// AS Jan 20/03 - Don't include if S4COMPRESS not defined
   ...
   int c4uncompress( CODE4 *c4, void *outputBuf, unsigned long *outLen, const void *inputBuf, unsigned long inputLen )
   {
      ...
      // return c4->uncompress( outputBuf, outLen, inputBuf, inputLen ) ;
      assert5port( "added compressed memo entries support" ) ;
...
int S4FUNCTION code4initLow( CODE4 *c4, const char *defaultProtocol, long versionId, long structSize, char skipCom )
{
   ...
   if ( skipCom != 1 )   /* for debug/ats, don't do stats/delay writes/advance reads */
   {
      ...
      #if defined( S4PREPROCESS_FILE ) && !defined( S4SERVER ) && !defined( S4ENCRYPT_DLL )
   ...
   c4->autoIncrementStart = 1.0 ;
      ...
      #if defined( S4OFF_MULTI ) && defined( S4STAND_ALONE )
...
CODE4 *S4FUNCTION code4allocLow( int doInit, const char *defaultProtocol, long versionId )
{
   ...
   if ( doInit == 1 )
   {
      ...
      int rc = code4initLow( c4, defaultProtocol, versionId, sizeof( CODE4 ), 0 ) ;
      if ( rc < 0 )
      {
...
{
   #if !defined(S4STAND_ALONE) || !defined(S4FILE_EXTENDED) || !defined(S4FOX)  // CS 2002/11/21 remove unnecessary conditions, improve logic
...
// AS Oct 22/02 - make available to be called in multi-user (if user calls directly)
   ...
   int code4validateAddOpen( CODE4 *c4, const char *fullPathTableName, Bool5 isTempIn )
   {
      ...
      */
      ...
      #ifndef S4OFF_MULTI
         if ( rc == 0 )
            d4unlockLow( valid, data4clientId( valid ), 0 ) ;
      #endif
   ...
   int S4FUNCTION code4validate( CODE4 *c4, const char *validateTableName, Bool5 deleteTemp )
   {
      ...
      */
      ...
      #ifdef E4ANALYZE
         /* LY 2003/07/09 : added (char *) to second param (compiler error on Linux) */
...
int S4FUNCTION code4additionalFunction( CODE4 *c4, long functionNumber, void *infoIn, long infoLenIn, void *infoOut, long *infoLenOut )
{
   ...
   void code4logInfoDo( CODE4 *c4, char *info )
   {
      ...
      static int c4encryptLoad( CODE4 *c4, short numBits )
      {
         switch( numBits )
         {
            ...
            case 256:
               break ;
            default:
               return error4describe( c4, e4parm, E91004, "encryption num bits set to invalid value (must be 8, 128, 192 or 256", 0, 0 ) ;
         }
         ...
         if ( c4->encrypt == 0 )
         {
            ...
            #endif
            //   c4getFuncPtr( encrypt->encryptDll, "encrypt4encryptHook", (void **)&(encrypt->encryptHook), &rc ) ;
            //   if ( rc != 0 || encrypt->encryptHook == 0 )
            //   {
            //      c4encryptUnload( c4 ) ;
            //      return error4describe( c4, e4notSupported, E91001, "Missing function in encryption dll", 0, encryptDllName ) ;
            //   }
            #ifdef S4ENCRYPT_COM
               ...
               c4getFuncPtr( encrypt->encryptDll, "code4encryptComInitUndoLow", (void **)&(encrypt->encryptComInitUndo), &rc ) ;
      ...
      int S4FUNCTION code4encryptInit( CODE4 *c4, const void *key, short keyLen )
      {
            ...
            assert5( c4->encrypt->preprocess == 0 ) ;  // not assigned yet
            assert5( c4->encrypt->preprocessInit == 0 ) ;  // not assigned yet
            return c4->encrypt->encryptInit( c4, key, keyLen, &c4->encrypt->blockSize, &(c4->encrypt->preprocess), &(c4->encrypt->preprocessInit) ) ;
         #else
            return e4notSupported ;
         #endif
      }
   ...
   short code4encryptFileGet( CODE4 *c4 )
   {
            ...
            if ( c4->encrypt == 0 )  // not available (yet) - usually with config.dbf
               return 0 ;
         #else
            if ( c4->encrypt == 0 )  // if not initialized then initialize
            {
               // we initializEncryption with a default value if not enabled
               return code4encryptInit( c4, " ", 1 ) ;
            }
         #endif
   ...
   static void code4encryptCodeInitUndo( CODE4 *c4 )
   {
      ...
      int con4lowPostProcess( CODE4 *c4, void *preprocess, unsigned char *message, int len )
      {
         ...
         return c4->encrypt->conLowDecrypt( preprocess, message, len ) ;
      }
   ...
   void code4fileDecrypt( FILE4 *f4, long pos, const void *ptrIn, long ptrInLen, void *ptrOut )
   {
      ...
      int code4serverPreprocessInit( CODE4 *c4, const char *userId, const char *password )
      {
            ...
            return c4->encrypt->serverEncryptInit( c4, userId, password ) ;
         #else
            return 0 ;
         #endif
      }
   ...
   //       CODE4 *c4 = f4->codeBase ;
   //       assert5( c4->encrypt != 0 ) ;
   //       assert5( c4->encrypt->encryptHook != 0 ) ;
   //       c4->encrypt->encryptHook( c4->encrypt->preprocess, c4->encrypt->preprocessInit, f4, pos, ptrIn, ptrInLen, ptrOut ) ;
   //    #endif
   // }
