      ...
      for ( ;; )
      {
         ...
         if ( messageLen == message1->messageLen )
         {
            ...
            if ( message1->currentPtr != message1->message )
            {
               ...
               {
                  #ifdef S4ENCRYPT_DLL
                     con4lowPostProcess( c4, connectBuffer->connectLowPtr->preprocess, (unsigned char *)message1->currentPtr, messageLen ) ;
                  #else
            ...
            {
               #ifdef S4COMPRESS
                  message1->messageLen = connect4lowUncompress( (unsigned char *)message1->currentPtr, actualMessageLen, message1->messageBufferLen ) ;
         ...
         if ( messageLen < message1->messageLen )
         {
            ...
            NET4MESSAGE *newMessage = (NET4MESSAGE *)u4alloc( size );
            if ( newMessage == 0 )
            {
            ...
            message1 = 0 ;
               ...
               {
                  #ifdef S4ENCRYPT_DLL
                     con4lowPostProcess( c4, connectBuffer->connectLowPtr->preprocess, (unsigned char *)newMessage->currentPtr, messageLen ) ;
                  #else
            ...
            if ( compressed )
            {
                  ...
                  newMessage->messageLen = connect4lowUncompress( (unsigned char *)newMessage->currentPtr, actualMessageLen, maxLen ) ;
         ...
         for ( ;; )
         {
            ...
            if ( c4->readMessageNumBuffers == 1 && (message1 != c4->extraReadMessage))   // we already have 'message1', need to post another
            {
               ...
               else
               {
                  ...
                  {
                     int size = sizeof( NET4MESSAGE ) + message1->messageBufferLen ;
                     c4->extraReadMessage = (NET4MESSAGE *)u4alloc( size );
                     if ( c4->extraReadMessage == 0 )
                     {
                        error4( c4, e4memory, E96921 ) ;
                        return 0 ;
                  ...
                  else
                  {
                     ...
                     int size = sizeof( NET4MESSAGE ) + message1->messageBufferLen ;
                     extraMess = (NET4MESSAGE *)u4alloc( size );
                     if ( extraMess == 0 )
                     {
                        error4( c4, e4memory, E96921 ) ;
                        return 0 ;
            ...
            if ( message2->messageLen >= requiredLen )
            {
               ...
               if ( compressed )
               {
                     ...
                     message1->messageLen = connect4lowUncompress( (unsigned char *)message1->currentPtr, actualMessageLen, maxLen ) ;
                     assert5( message1->messageLen >= 0 ) ;
                  #else
                     error4( 0, e4info, E96914 ) ;  // not supported, should never happen
                     return 0 ;
   ...
   */
   ...
   connect4lowExitExclusive( connectBuffer ) ;
      ...
      while ( left > 0 )
      {
         ...
         assert5( mess->messageLen >= 0 ) ;  // AS Sept 26/02 - allow length of 0 as well
...
*/
   ...
   c4 = connectBuffer->cb ;
      ...
      rc = connect4lowWrite( connect4bufferLowGet( connectBuffer ), connectBuffer->workingWriteBuffer, connectBuffer->workingWritePos, c4->doCompress ) ;
      if ( rc )
      {
         connect4bufferDisconnect( connectBuffer ) ;
         connectBuffer->communicationError = e4connection ;
