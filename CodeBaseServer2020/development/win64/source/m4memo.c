...
int S4FUNCTION code4memoCompress( CODE4 *c4, short flag )
{
      ...
      #if defined( S4OFF_MEMO ) || ( defined( S4STAND_ALONE ) && !defined( S4FOX ) ) || !defined( S4COMPRESS )
   ...
   int dfile4memoCompress( DATA4FILE *data, DATA4 *d4 )
   {
      ...
      MEMO4FILE newFile ;
         ...
         code4setPreprocessFile( c4, data->memoFile.file.preprocessed ) ;  // CS 2000/04/30 temp memo file must have same preprocess as original
      #endif
