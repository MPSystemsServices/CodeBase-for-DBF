...
#ifdef S4CLIENT
/*
   int d4skipFetchBuffer( DATA4 *data, int nSkip )
   {
      // fetch a block of records from the server and place into a buffer

      CODE4 *c4 = data->codeBase ;

      // now, we may need to fetch all the rest of the records...
      if ( data->readRecsToBuf != 0 )
      {
         assert5( data->readAdvanceBuf != 0 ) ;
         // read to fill in the buffer...
         data->readBufNum = 0 ;
         char *pos = data->readAdvanceBuf ;
         short rc ;
         for ( ;; )
         {
            if ( lenLeft < 0 )
            ...
            if ( lenLeft == 0 )
               rc = r4done ;  // r4done is used to indicate no errors...
            else
               rc = htons5(*((short *)outPos)) ;

            if ( rc != 0 )  // we are done, non-0 rc...
            {
               lenLeft -= sizeof( short ) ;
               if ( lenLeft != 0 )
                  return error4( c4, e4info, E94802 ) ;

               // if ( rc == r4bof || rc == r4eof )
               //    data->readBufRcSave = rc ;
               // else
               //    data->readBufRcSave = 0
                  ;
               data->readBufPos = 0 ;  // at start of buffer
               if ( nSkip < 0 )
                  data->readBufDirection = -1 ;
               else
                  data->readBufDirection = 1 ;
            ...
            outPos += sizeof( short ) ;
            long recNo = htonl5(*((long *)outPos)) ;
            pos += sizeof( long ) ;
            // AS May 29/02 - changed d4recWidth to be an exposed only function due to record length changes with aes encryption
            memcpy( data->record, pos, dfile4recWidth( data->dataFile ) ) ;
            data->recNum = recNo ;
            data->readBufRecNoOn = recNum ;
            return 0 ;
         }
      }
      else // better be no length left
      {
         if ( lenLeft > 0 )
            return error4( c4, e4info, E94802 ) ;
      }
   ...
   int d4skipFetchLow( DATA4 *data, short mode, const long nSkip, const char *seekKey, short keyLen, double dkey )
   {
      ...
      for ( Bool5 isDone = 0 ; isDone == 0 ; )
      {
         ...
         switch ( code )
         {
            ...
            case STREAM4UNLOCK_DATA:
               {
                  long unlockClientId = connect4receiveLong( connect ) ;
                  long unlockServerId = connect4receiveLong( connect ) ;
                  d4unlockClientData( tran4data( &(c4->c4trans.trans), unlockServerId, unlockClientId )) ;
               }
               break ;
      ...
      for( ;; )  // handle the bufferring if required
      {
         if ( data->batchRead.readBufNum != 0 )  // non-first record, haven't read rc yet
         {
            ...
            if ( rc != 0 )  // means done (reached eof, error, bof, etc...)
            {
               // if seekNext, r4entry means continue to get 'record after'
               // if ( seekKey == 0 || rc != r4entry )
                  return saveRc ;
            }
         }
         ...
         if ( data->batchRead.readBufNum == data->batchRead.readRecsToBuf ) // done
            return saveRc ;
         // if ( rc == r4entry )  // means after seek, had r4entry, just got last entry, done now
         //    return saveRc ;
      }
