...
enum Collate4name
{
   ...
   */
   ...
   // AS Jan 8/03 - Added upper case collation for croatian
   #define NUM4AVAIL_COLLATION_ENTRIES 6
...
// AS Nov 26/02 - for data file compression
   ...
   {
      // the header
      short version ;    // what version of the compression is this?
...
typedef struct FILE4St
{
   ...
   char type ;           /* dbf, index, other */
      ...
      */
      ...
      long uBufFilePos ;  // what file position is associated with the compressed buffer
      long uBufFileLen ;  // and what length is that buffer?
      long physicalFileLen ;  // the actual physical file length (compressed)
   #else
      void *space35 ;
      Bool5 space36 ;
...
// #endif
      ...
      typedef struct CONNECT4LOWSt
      {
         ...
         short minLength ;       // minimum length before compression is considered
               ...
               void *preprocessInit ;
               short blockSize ;
               struct CODE4St *c4 ;      // required for dll encryption
            #else
...
// typedef int S4FUNCTION2 INT4PTR_PTR_PTR_ULONG( void S4PTR *, unsigned long S4PTR *, const void S4PTR *, unsigned long ) ;
   ...
   typedef int   INT4PTR_PTR_INT( void S4PTR *, void S4PTR *, int ) ;
...
typedef struct INDEX4St
{
   ...
   /* LY 2003/07/25 : commented out !S4OFF_TRAN because of AS Feb 7/03 in i4create.c */
   // #ifndef S4OFF_TRAN
      int isValid ;
   // #endif
   #ifndef S4CLIPPER
