   ...
   int tfile4init( TAG4FILE *t4, INDEX4 *i4, FILE4LONG filePos, unsigned char *name )
   {
      ...
      if ( *header->sortSeq == '\0' )
      {
         ...
         else if ( c4strcmp( header->sortSeq, "GENERAL" ) == 0 )
         {
            ...
            if ( t4->indexFile->dataFile->codePage == cp0 ) // code page 0 uses windows ansi by default
            {
               if ( tfile4setCollatingSeq( t4, collate4generalCp1252, 0 ) < 0 )
                  return error4( c4, e4index, E84907 ) ;
            }
            if ( t4->indexFile->dataFile->codePage == cp437 )
            {
               if ( tfile4setCollatingSeq( t4, collate4generalCp437, 0 ) < 0 )
                  return error4( c4, e4index, E84907 ) ;
            }
            if ( t4->indexFile->dataFile->codePage == cp1250 )  // AS Dec 30/02 - not supported - must use non-general sorting (croatian)
               return error4( c4, e4index, E84907 ) ;
         }
      #endif
      ...
      if ( header->typeCode < 0x80 )  /* non-compound header; so expression */
      {
         ...
         {
            file4longAssignLong( pos, filePos ) ;
            file4longAdd( &pos, i4blockSize( i4file ) + header->exprLen ) ;
      ...
      {
         // AS 03/08/00 in this case, not callint tfile4initSeekConv() for the tagIndex, so not setting the pChar
      ...
      switch( type )
      {
         ...
         case r4log:
            ...
            case r5dbTime:
               ...
               t4->pChar = '\0' ;
               break ;
            case r4dateTime:   // supports 3 decimal fraction
