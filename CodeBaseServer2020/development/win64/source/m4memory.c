...
#define mem4numTypes 10
   ...
   int v4print = 0 ; /* if v4print == -1 no output, if 0 only u4alloc/u4free output, if 1 all output  */
...
typedef struct
{
   ...
   #if defined( S4WIN32 ) || defined( S4LINUX ) /* LY 2002/02/20 : added S4LINUX */
...
#ifdef S4SEMAPHORE
#if !defined( S4WIN32 ) && !defined(S4LINUX) /* added !S4LINUX */
...
#endif /* S4WIN32 && S4LINUX */
#ifdef S4LINUX /* LY 2002/02/20 */
   /* LY 2003/07/09 : removed static to avoid odd Linux compiler error */
...
#ifdef S4TESTING
   static int s4allocOff = 0 ;  /* LY 2003/07/09 : added static */
#endif
...
void S4FUNCTION mem4init( void )
{
   ...
   // capabilities...
   ...
   #ifdef S4SEMAPHORE
      #if defined( S4WIN32 ) || defined( S4LINUX ) /* LY 2002/02/20 : addes S4LINUX */
         if ( resetInProgress == 0 )
         {
               ...
               #endif
            #else
               pthread_mutex_init( &critical4memory, 0 ) ;
               /* LY 2003/07/11 : t4calc.c locking itself out under E4DEBUG; initialize in declaration above
