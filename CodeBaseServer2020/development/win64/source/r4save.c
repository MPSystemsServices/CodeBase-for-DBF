...
/*   EXPR4 *expr; */
   ...
   report->report_name = (LPTSTR)u4allocEr( report->codeBase, (strlen(buffer)+1) * sizeof(TCHAR) ) ;
   ...
   report->report_caption = (LPTSTR)u4allocEr( report->codeBase, (strlen(buffer)+1) * sizeof(TCHAR) ) ;
   if ( !report->report_caption )
   {
      report4free( report, 1, 1 ) ;
      return NULL ;
   ...
   #ifdef S4UNICODE
      c4atou(buffer, report->report_caption, (strlen(buffer)+1) * sizeof(TCHAR) ) ;
   #else
...
int S4FUNCTION report4save_foo( REPORT4 *report, char *file_name, int path, char *buf, char *name_buf )
{
   ...
   report->report_file_name = (char *)u4allocEr( report->codeBase, strlen(file.nameBuf) + 1 ) ;
   if ( !report->report_file_name )
      return -1 ;
   strcpy( report->report_file_name, file.nameBuf ) ;
