...
static void d4openConcludeAllocateRecord( DATA4 *d4 )
{
   ...
   if ( ( ( recWidth2 + 50L ) * 2L + (long)sizeof( FIELD4 ) * (long)d4->dataFile->nFields )
   > (long)UINT_MAX )  /* try allocating records and fields together, 50 bytes for overhead */
   {
...
void d4openConcludeSetupTransactions( DATA4 *d4 )
{
   ...
   if ( code4transEnabled( c4 ) )
   {
      ...
      if ( c4->createTemp == 1 || d4->dataFile->file.isTemp == 1 )
      {
...
DATA4 *S4FUNCTION d4open( CODE4 *c4, const char *name )
{
   ...
   if ( d4 == 0)
      ...
      Bool5 changedEncryptionFlag = 0 ;  // true if we successfulling opened in opposite mode - need to set for opening memo/index files
   ...
   #ifdef S4ENCRYPT_HOOK
      short oldEncrypt = c4->encrypt ;
      if ( changedEncryptionFlag )  // we changed it before, so swap it again here.
         c4->encrypt = ( oldEncrypt ? 0 : 1 ) ;  // just put to the opposite number
   #endif
...
static void dfile4accesses( const DATA4FILE *d4, int *readMode, int *accessMode, int *otherUsers )
{
   ...
   *otherUsers = 0 ;
   ...
   server4clientListReserve( d4->c4->server ) ;

   for ( client = 0 ;; )
   {
      client = server4clientGetNext( d4->c4->server, client ) ;
      if ( client == 0 )
         break ;
      ...
      for ( data = 0 ;; )
      {
         ...
         if ( data->dataFile == d4 )
         {
            *otherUsers = 1 ;
            ...
            {
               server4clientListRelease( d4->c4->server ) ;
               return ;
            }
         }
      }
...
#ifndef S4SERVER
   static int data4reopenVerifyOnlyOneInstance( const DATA4FILE *d4 )
   {
      CODE4 *c4 = d4->c4 ;
...
static int data4reopenVerifyDesiredAccessAvail( const DATA4FILE *d4 )
{
   ...
   return canUpgrade ;
      ...
      static int index4attemptUpgrade( INDEX4FILE *i4 )
      {
         ...
         int rc = file4open( &indexFile, i4->codeBase, i4->file.name, 1 ) ;
   ...
   static int data4reopenAttemptUpgrade( DATA4FILE *d4, int upgradeRequired )
   {
      ...
      // investigate further
         ...
         c4->accessMode = d4->file.lowAccessMode ;


         // upgrade data file
         FILE4 dataFile, memoFile ;
         int rc = file4open( &dataFile, c4, d4->file.name, 1 ) ;
         ...
         if ( d4->nFieldsMemo != 0 )
         {
            int rc = file4open( &memoFile, c4, d4->memoFile.file.name, 1 ) ;
...
static int dfile4setupFromInfo( DATA4FILE *d4, const char *name, const CONNECTION4OPEN_INFO_OUT *dataInfo, DATA4 *data )
{
   ...
   len2 += d4->infoLen ;
      ...
      if ( c4->autoOpen == 1 && ntohs5(dataInfo->numTags) > 0 )
      {
         ...
         if ( client4indexSetup( c4, data, d4, ntohs5(dataInfo->numTags), connection4data( connection ) + len2, (unsigned int)connection4len( connection ) - len2, indexName, 0 ) < 0 )
            return -1 ;
      }
   #endif
...
DATA4FILE *dfile4open( CODE4 *c4, DATA4 *data, const char *nameIn, char **info )
{
   ...
   d4->userCount = 1 ;
         ...
         #endif
      #endif
      if ( file4open( &d4->file, c4, name, 1 ) != 0 )
      {
         dfile4close( d4 ) ;
         return 0 ;
      }
