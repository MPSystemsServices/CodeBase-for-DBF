...
int relate4unpack( RELATION4 *relation, CONNECTION4 *connection )
{
   ...
   for( relate = &relation->relate ;; )
   {
      ...
      relate->data->recNum = ntohl5(relate->data->recNum) ;
         ...
         for( relateOn = &relate->relation->relate ;; )
         {
            if ( relateOn == 0 )
               ...
               if ( relateOn->dataOld->dataFile->fileServerLock == data4serverId( relateOn->dataOld ) )  /* check for duplicate lock */
               {
                  ...
                  rc = dlockAllInternal( relateOn->dataOld->dataFile, data4clientId( relateOn->dataOld ), data4serverId( relateOn->dataOld ), relateOn->dataOld, 0 ) ;
            #else
...
/* not S4CLIENT */
static int relate4lookupRecno( RELATE4 *relate, const signed char direction, long *recno )
{
...
/* not S4OFF_INDEX, not S4CLIENT */
static int relate4lookupTag( RELATE4 *relate, const signed char direction, long *recno )
{
   /*
   ...
   if ( relate->relationType == relate4approx || rc == 0 )
   {
      ...
      if ( direction < 0 && rc == 0 && relate->relationType == relate4scan )  /* look for last one */
         for( ;; )
         {
            ...
            #ifdef S4FOX
               // AS 07/27/99 -- this has been superseded by more simple collations for foxPro
               // if ( u4keycmp( tfile4keyData( tagFile )->value, ptr, (unsigned int)len, (unsigned int)relate->dataTag->tagFile->header.keyLen, 0, &tagFile->vfpInfo ) )
               if ( u4keycmp( tfile4keyData( tagFile )->value, ptr, (unsigned int)len, (unsigned int)relate->dataTag->tagFile->header.keyLen, 0, collation4get( tagFile->collateName ) ) )
            #else
...
int S4FUNCTION relate4matchLen( RELATE4 *relate, const int matchLenIn )
{
   ...
   */
      ...
      len = expr4keyLen( relate->masterExpr ) ;
         ...
         {
            /* LY 99/03/09 */
            len = ( expr4keyLen( relate->masterExpr ) < tfile4keyLen( relate->dataTag->tagFile ) ?
               expr4keyLen( relate->masterExpr ) : tfile4keyLen( relate->dataTag->tagFile ) ) ;
         }
         else
      #endif
            len = expr4keyLen( relate->masterExpr ) ;
...
static int relate4sort( RELATE4 *relate )
{
   ...
   */
   ...
   /* AS sort has nothing to do with tag and ordering because tag is not coming into this.
      if ( expr4key( sortExpr, &sortKey, relate->dataTag == 0 ? 0 : relate->dataTag->tagFile ) < 0 ) */
   ...
   if ( relation->skipBackwards )
      if ( file4tempLow( &relation->sortedFile, c4, 1, 1, 0 ) < 0 )
      {
         u4free( relation->otherData ) ;
         relation->otherData = 0 ;
         return -1 ;
...
static int relate4sortGetRecord( RELATION4 *relation, const long num )
{
   ...
   char *other = 0 ;
   R4DATA_LIST *linkOn ;
   S4LONG j, numLeft ;  /* L.Y. 1999/2/5 : sort4get() takes S4LONG* as 2nd
                           param, which may be int* under 64-bit */
   FILE4LONG pos ;
   #ifdef S4DATA_ALIGN
      S4LONG longPtr ;
   #endif
...
int relate4clientInit( RELATE4 *master )
{
   ...
   if ( master->relation->needsFreeing == 1 )
      info->relationId = htonl5(master->relation->relationId) ;
   else
      info->relationId = 0 ;
   ...
   info->relation.skipBackwards = htons5(master->relation->skipBackwards) ;
   info->bitmapDisable = htons5(master->relation->bitmapDisable) ;
   ...
   master->relation->relationId = ntohl5(out->relationId) ;
