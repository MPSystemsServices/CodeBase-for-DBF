...
#include "d4all.h"
   ...
   int tfile4addDo( TAG4FILE *t4, const unsigned char *keyInfo, unsigned long rec, short int errUnique )
   {
      ...
      if ( rc < 0 )
         ...
         sprintf( dump, "id = %ld, add tag key: fb = %uld, rec = %ld, key = %s", (long)i4, i4block->fileBlock, (long)recIn, (char *)keyInfo ) ;
         u4writeErr( dump, 1 ) ;
         u4writeErr( outTime, 0 ) ;
         i4block = (B4BLOCK *)l4first( &t4->blocks ) ;
         sprintf( dump, "add extra info: rc = %ld, root block: fb = %uld, curKey # = %ld",
                  rc, i4block->fileBlock, i4block->keyOn ) ;
         u4writeErr( dump, 1 ) ;
         char cnt = 2 ;
         for( ;; )
         {
