...
FIXED4MEM S4FUNCTION u4allocFixedDefault( CODE4 *c4, long n )
{
   ...
   */
      ...
      c4memset( &fixedMem, 0, sizeof( FIXED4MEM ) ) ;
      ...
      c4memset( (void *)s4protected( fixedMem ), 0, (size_t)n ) ;
      return fixedMem ;
   #else
...
void S4FUNCTION mem4init( void )
{
   ...
   FIXED4MEM S4FUNCTION u4printAllocFixed( CODE4 *a, long b, const char *file, int line )
   {
      mem4start( 0 ) ;
      code4memFileNameSet( file ) ;
      code4memLineNoSet( line ) ;
      FIXED4MEM val = u4allocFixedDefault( (a), (b) ) ;
      mem4stop( 0 ) ;
      return val ;
   }
   ...
   void S4FUNCTION u4printFreeFixed( FIXED4MEM a, const char *file, int line )
   {
      mem4start( 0 ) ;
      code4memFileNameSet( file ) ;
      code4memLineNoSet( line ) ;
      u4freeFixedDefault( a ) ;
      mem4stop( 0 ) ;
      return ;
   }
