...
#include "d4all.h"
   ...
   int S4FUNCTION code4infoRetrieve( CODE4 *c4, S4LONG *memAlloc, unsigned short *nClients, S4LONG *elapsedSeconds, int *nOpenFiles )
   {
      ...
      *memAlloc = htonl5( out->memAlloc ) ;
      *nClients = htons5( out->numClients ) + htons5( out->numClientsOdbc ) ;
      *elapsedSeconds = htonl5( out->elapsedSeconds ) ;
      *nOpenFiles = htons5( out->nOpenFiles ) ;
