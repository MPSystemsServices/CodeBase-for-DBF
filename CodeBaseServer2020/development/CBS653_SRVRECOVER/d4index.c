...
#if defined( TIME4STATUS ) && !defined( S4OFF_INDEX ) && !defined( S4OFF_WRITE ) && !defined( S4UNIX )
   #include <process.h>
#endif
...
INDEX4FILE *dfile4index( DATA4FILE *data, const char *indexName )
{
...
#ifdef S4WIN32
   #if defined( TIME4STATUS ) && !defined( S4OFF_INDEX ) && !defined( S4OFF_WRITE ) && !defined( S4CLIPPER )
      typedef struct
      {
         DATA4 *data;
   ...
   {
      #ifndef S4OFF_INDEX
         #if defined( TIME4STATUS ) && !defined( S4CLIPPER )
            if ( callback == 0 )
               ...
               while (r4info->callbackStarted == 0 || data->codeBase->actionCode == ACTION4NONE)
                  Sleep(0);  // wait for d4reindexMonitor to start
            #endif  // !OFF_WRITE
         #else  // TIME4STATUS
            callback(0.0);
