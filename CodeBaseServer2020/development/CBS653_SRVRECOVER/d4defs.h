...
//   #define EXCEPTION4REINDEX
   ...
   #ifndef EXCEPTION4REINDEX
      #ifndef S4OFF_EXCEPTION_REINDEX
         #define EXCEPTION4REINDEX
      #endif
   #endif
#endif
...
// S4ODBC_ENABLED is used to build a CodeServer which supports ODBC clients
   ...
   // cannot use shared file in c/s because it locks all other users out.  Therefore turn off...
   // share trans file in simba multi multi-user server implementation...
   // for now at least, no support for transactions
   #define S4OFF_TRAN
   // #ifdef S4STAND_ALONE
   ...
   //    #endif
   // #endif
   // don't leave the lock in place for the whole transaction because with ODBC transactions are always going, even
   // on read operations
   // #define S4TRANS_FULL_LOCK_OFF  -- currenlty does not work
#endif

#ifdef S4STAND_ALONE
   // in stand-alone mode, the transaction file is shared between the users.
   ...
   // odbc enabled server does normal transaction support - the ODBC engine just does not support transactions.
   // the concurrent server must perform transactions in a multi-user method as well
   // #define S4TRANS_FILE_SHARED
   // don't leave the lock in place for the whole transaction because with ODBC transactions are always going, even
   // on read operations
   // #define S4TRANS_FULL_LOCK_OFF  -- currenlty does not work
   ...
   #if !defined( S4DLL_BUILD ) || defined( S4WINCE ) || defined( PASCAL_DOS ) || defined( S4SERVER )
...
#if !defined(S4NO_LONGLONG) && defined(S4UNIX)
...
#define TIME4OUT 5
   ...
   #if !defined( S4WINSOCK ) && !defined( S4BERKSOCK ) && !defined( S4MAC_TCP )
...
/* AS 07/27/99 -> we don't support cp850, so remove the value...
#define cp850    2
*/
#define cp1252   3
...
#define collate4special 1002
   ...
   #define SHARE4MEM_LEN  SHARE4MEM_LARGE_LEN + SHARE4MEM_ACCEPT_LEN + SHARE4MEM_CONFIG_NAME_LEN
#endif
...
/* AS Aug 22/01 - Moved these from e4expr.h since they are exposed to user, and need for dual dlls */
