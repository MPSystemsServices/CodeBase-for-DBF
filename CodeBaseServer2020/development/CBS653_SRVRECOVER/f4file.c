...
static unsigned file4readLowDo( FILE4 *f4, FILE4LONG pos, void *ptr, unsigned len )
{
   ...
   if ( file4longError( pos ) == (unsigned long)-1L )
   {
      ...
      if ( f4->encrypted )
         code4decryptHook( f4->codeBase, f4->codeBase->encryptInit, f4, file4longGetLo( pos ), ptr, urc, ptr ) ;
   #endif
   ...
   /* 05/09/96 AS first, must flush the files to disk to avoid delay-writes
      ...
      /*#ifdef S4MACINTOSH
         ...
         if ( file4open ( keep, c4, fromName, 0 ) )
            return -1 ;
      #else */  // CJ - 14/03/00 - Have the same code for macintosh as windows. made a change to u4rename.
