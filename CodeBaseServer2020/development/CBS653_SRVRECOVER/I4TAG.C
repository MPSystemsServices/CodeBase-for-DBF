         ...
         int tfile4down( TAG4FILE *t4 )
         {
            ...
            if ( blockOn == 0 )    /* Read the root block */
            {
               if ( root4needsReading( t4->header.root ) )
               {
                  ...
                  #ifdef I4PRINT
                     char dump[80] ;
                     sprintf( dump, "tfile4down read root: i4 = %ld, root = %ld",
                              (long)t4->index, (long)t4->header.root ) ;
                     u4writeErr( dump, 1 ) ;
                  #endif
               }
      ...
      int tfile4setCollatingSeq( TAG4FILE *t4, Collate4name collateName, Bool5 setHeader )
      {
         ...
         */
         ...
         if ( collateName == collate4none )
         {
            ...
            assert5( t4->codeBase->collatingSequence == sort4general ) ;  // only other value we support...
      ...
      */
      ...
      B4BLOCK *tfile4split( TAG4FILE *t4, B4BLOCK *oldBlock )
      {
         ...
         oldBlock->changed = 1 ;
            ...
            oldBlock->keyOn = oldBlock->keyOn - b4numKeys( newBlock ) ;
               ...
               sprintf( dump, "id = %ld, split: old-fb = %uld, new-fb = %uld", (long)t4->indexFile, oldBlock->fileBlock, newBlock->fileBlock ) ;
               u4writeErr( dump, 1 ) ;
               u4writeErr( outTime, 0 ) ;
               B4KEY_DATA *keyData = b4key( newBlock, 1 )  ;
               sprintf( dump, "          1st split new fb key: recno = %ld, key = %s", (long)keyData->num, (char *)keyData->value ) ;
               u4writeErr( dump, 1 ) ;
            #endif
         #endif
      ...
      int tfile4update( TAG4FILE *t4 )
      {
         ...
         if ( t4->rootWrite )
         {
            #ifdef I4PRINT
               char dump[80] ;
               sprintf( dump, "rootWriting: i4 = %ld, root = %ld",
                        (long)t4->index, (long)t4->header.root ) ;
               u4writeErr( dump, 1 ) ;
            #endif
            #ifdef S4BYTE_SWAP
