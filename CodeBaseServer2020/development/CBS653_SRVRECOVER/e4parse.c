               ...
               case E4FIELD_STR_CAT:
                  ...
                  case E4FIELD_MEMO:
                     if ( p4->expr.tagPtr )  /* max size is key size */
                     {
                        ...
                        #endif
                        /* AS fix #155 - allows complex memo expressions
                           if the memSizeMemoExpr is set.
                        */
                        if ( (long)codeBase->memSizeMemoExpr < length[numParms] )
...
static int expr4parseStrZeroFunction( E4PARSE *p4, int *functionNumber, int numParms, int *infoI1, int *infoLen )
{
   ...
   // STRZERO( numericVal, numericLen ) only is valid...
...
//   if ( secondParamaterInfo->functionI != E4DOUBLE )
//   {
//      if ( p4->codeBase->errExpr )
//         return error4describe( p4->codeBase, e4notConstant, E90909, p4->expr.source, 0, 0 ) ;
//      return e4notConstant ;
...
static int expr4parseRightFunction( E4PARSE *p4, int numParms, int *infoI1, int *infoLen )
{
   ...
   // secondParamaterInfo will point to the 2nd (right) paramater
   E4INFO *secondParamaterInfo = firstParamaterInfo - 1 ;   /* get the constant/field len */
...
static int expr4parseDateTimeParmFunction( E4PARSE *p4, int numParms, int *infoI1 )
{
...
int expr4parseFunction( E4PARSE *p4, const char *functionName, const int functionNameLen )
{
   ...
   */
   ...
   if ( numParms >= 3 && numParms <= 6 && functionNumber == E4DATETIME )
      numParms = expr4parseDateTimeParmFunction( p4, numParms, &infoI1 ) ;
   ...
   {
      if( functionNumber == E4DTOC && numParms == 2 )
      {
         e4functionPop( &p4->expr ) ;
         numParms-- ;
...
static int expr4parseField( E4PARSE *p4, const char *inputName, int len )
{
   ...
   char fieldName[11] ;
   c4memmove( fieldName, startPtr, (size_t)len ) ;
   fieldName[len] = 0 ;
   FIELD4 *fieldPtr = 0 ;
   ...
   if ( !c4->errExpr )
   {
      ...
      c4->errFieldName = 0;
      fieldPtr = d4field( basePtr, fieldName ) ;
