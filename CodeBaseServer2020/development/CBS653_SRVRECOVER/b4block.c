   ...
   *to = *--from ;
   ...
   int b4doFlush( B4BLOCK *b4 )
   {
      ...
      file4longMultiply( fPos, I4MULTIPLY ) ;
         ...
         sprintf( dump, "b4flush - 1st entry: i4 = %ld, fileBlock: %uld, recNo = %ld, key = %s", (long)b4->tag->indexFile,
            (unsigned long)b4->fileBlock, (long)keyData->num, (char *)keyData->value ) ;
         u4writeErr( dump, 1 ) ;
   ...
   /* S4MDX */
      ...
      int b4remove( B4BLOCK *b4 )
      {
         ...
         int leftLen = b4->tag->indexFile->header.blockRw - sizeof( b4->nKeys ) - sizeof( b4->dummy ) - ( b4->keyOn + 1 ) * b4->tag->header.groupLen ;
            ...
            sprintf( dump, "remove - removed entry: i4 = %ld, #keys = %ld, keyOn = %uld, fileBlock: %uld, recNo = %uld, key = %s", (long)b4->tag->indexFile,
               (long)b4->nKeys, (long)b4->keyOn, b4->fileBlock, keyData->num, (char *)keyData->value ) ;
            u4writeErr( dump, 1 ) ;
            u4writeErr( outTime, 0 ) ;
         #endif
         ...
         #ifdef I4PRINT_BLOCK
            keyData = b4key( b4, 0 ) ;
            sprintf( dump, "b4remove - 1st entry before: i4 = %ld, fileBlock: %uld, recNo = %uld, key = %s", (long)b4->tag->indexFile,
               b4->fileBlock, keyData->num, (char *)keyData->value ) ;
            u4writeErr( dump, 1 ) ;
            u4writeErr( outTime, 0 ) ;
         #endif
         ...
         #ifdef I4PRINT_BLOCK
            keyData = b4key( b4, 0 ) ;
            sprintf( dump, "b4remove - 1st entry after: i4 = %ld, fileBlock: %uld, recNo = %uld, key = %s", (long)b4->tag->indexFile,
               b4->fileBlock, keyData->num, (char *)keyData->value ) ;
            u4writeErr( dump, 1 ) ;
            u4writeErr( outTime, 0 ) ;
         #endif
   ...
   /* S4MDX */
   int b4skip( B4BLOCK *b4, int numSkip )
   {
      #ifdef E4PARM_LOW
         if ( b4 == 0 )
         {
   ...
   B4KEY_DATA *b4key( B4BLOCK *b4, const int iKey )
   {
      ...
      else /* branch */
      {
         ...
         b4->builtKey->num = (unsigned long)x4reverseLong( (void *)( ((unsigned char *)&b4->nodeHdr)
   ...
   int b4brReplace( B4BLOCK *b4, const unsigned char *str, const S4UNSIGNED_LONG r )
   {
      ...
      /* LY 2000/04/06 - unsigned long to S4UNSIGNED_LONG for 64-bit */
      S4UNSIGNED_LONG rec = (unsigned long)x4reverseLong( (void *)&r ) ;
   ...
   /* S4FOX */
   int b4skip( B4BLOCK *b4, int numToSkip )  // can only skip by int amount within block
   {
      ...
      int b4verify( B4BLOCK *b4 )
      {
         ...
         for ( i = 0 ; i < b4numKeys( b4 ) - 1 ; i++ )
         {
            ...
            #endif
               {
                  ...
                  #endif
                  #ifdef E4ANALYZE_ALL
                     return error4describe( b4->tag->codeBase, e4index, E80406, t4->alias, 0, 0 ) ;
                  #else
