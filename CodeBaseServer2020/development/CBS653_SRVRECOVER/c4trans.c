...
*/
   ...
   static int tran4fileVerify( TRAN4FILE *t4, int subs )
   {
         ...
         if ( rc < 0 )
            return rc ;
         return tran4fileLowVerify( &(t4->backup), subs ) ;
      #else
   ...
   int S4FUNCTION tran4fileAppend( TRAN4FILE *t4, LOG4HEADER *header, const char *transactionData, int doImmediateFlushing )
   {
         ...
         if ( t4->backup.validState )
         {
            ...
            if ( rc2 < 0 )
            {
               ...
               error4set( t4->primary.file.codeBase, 0 ) ;
            }
         }
         return rc ;
      #else
   ...
   int tran4fileLowClose( TRAN4FILE_LOW *t4 )
   {
      ...
      int saveRc = 0 ;
         ...
         // AS 10/21/99 -- in actual fact, the file never gets closed if there are any active
         // clients, so just verify this...

         // AS 09/19/00 - there is an active client if we are closing the backup
         // log file as part of the swap procedure, so allow this... (i.e. if not primary, may be backup or temporary create of extra backup)
         // assert5( server4clientGetFirst( t4->c4trans->c4->server ) == 0 ) ;
         assert5( server4clientGetFirst( t4->c4trans->c4->server ) == 0 || (&(t4->transFile->primary) != t4)) ;
         ...
         SERVER4CLIENT *client = (SERVER4CLIENT *)l4first( &t4->c4trans->c4->server->clients.list ) ;
         for ( ;; )
         {
            if ( client == 0 )
               break ;
      ...
      int saveErr = error4code( t4->c4trans->c4 ) ;
      ...
      if ( rc == 0 )
      {
         memcpy( d4record( data ), rec, (size_t)dfile4recWidth( data->dataFile ) ) ;
            ...
            for ( i = 0; i < data->dataFile->nFieldsMemo; i++ )
            {
               ...
               #ifdef S4DATA_ALIGN
                  memcpy(&ptrLen, tran4getData( trans, pos ), sizeof(unsigned int *) ) ;
               #else
   ...
   int S4FUNCTION tran4lowStart( TRAN4 *trans, long clientId, int doUnlock )
   {
      ...
      if ( trans->c4trans->enabled == 0 )
      {
         ...
         #else
            #ifndef S4UNIX /* LY 2001/09/13 */
               throw Err5internal( c4 ) ;  // should never get here - code below does not take backup server into account
         ...
         if ( c4trans->enabled == 0 )
         {
            ...
            if( c4->transFileName != 0 )
            {
               ...
               if ( rc == 0 )
               {
                  ...
                  {
                     #ifdef S4SERVER
                        rc = tran4fileLowOpen( &c4trans->transFile->primary, c4->transFileName ) ;
                     #else
                        rc = tran4fileOpen( c4trans->transFile, c4->transFileName ) ;
                     #endif
                  }
                  else
                  {
                     rc = tran4fileCreate( c4trans->transFile, c4->transFileName ) ;
                  }
                  if ( rc == 0 )
                     c4trans->enabled = 1 ;
                  else
