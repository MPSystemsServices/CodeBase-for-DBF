...
*/
   ...
   int S4FUNCTION tfile4lock( TAG4FILE *t4, const long serverId )
   {
            ...
            // FILE4LONG position ;
            // file4longAssign( position, L4LOCK_POS, c4->largeFileOffset ) ;
            // rc = file4lockInternal( &t4->file, file4longGetLo( position ), file4longGetHi( position ), 1L, 0 ) ;
   ...
   int S4FUNCTION tfile4unlock( TAG4FILE *t4, const long serverId )
   {
            ...
            if ( t4->fileLocked == serverId || ( t4->fileLocked != 0 && serverId == 0 ) )
            {
               ...
               // FILE4LONG position ;
               // file4longAssign( position, L4LOCK_POS, c4->largeFileOffset ) ;
               // rc = file4unlockInternal( &t4->file, file4longGetLo( position ), file4longGetHi( position ), 1L, 0 ) ;
