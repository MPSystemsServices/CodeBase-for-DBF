...
#include "d4all.h"
   ...
   static int i4addOneTag( INDEX4 *i4, const TAG4INFO *tagData )
   {
      ...
      else
      {
         ...
         for ( ; rc == 0 ; )
         {
            ...
            // tagFile->headerOffset /= i4multiplier( i4file ) ;
            // tagFile->headerOffset *= i4blockSize( i4file ) ;
            ...
            if ( rc == 0 )
            {
               ...
               file4longAdd( &pos, 3 * i4blockSize( i4file ) ) ;
   ...
   int S4FUNCTION i4tagAdd( INDEX4 *i4, const TAG4INFO *tagDataIn )
   {
      ...
      for( tagData = tagDataIn ;; tagData++)
      {
         if ( tagData->name == 0 )
            return 0 ;
         TAG4 *tag = t4create( i4->data, tagData, i4, 0 ) ;
         ...
         if ( t4reindex( tag ) != 0 )
         {
            int retCode = error4set( c4, 0 ) ;
            t4close( tag ) ;
            return retCode ;
         }

         // open group file and append entry... if tag create succeeded
         int rc = i4addEntryToGroupFile( i4, tagData->name ) ;
         if ( rc != 0 )
            return rc ;
      }
   }
   ...
   static void file4removePart( FILE4 *file, int startPos, int endPos )
   {
      ...
      file4longAssignLong( writePos, len ) ;
   ...
   void i4removeTagEntryFromGroupFile( INDEX4 *index, const char *fullPathTagNameIn )
   {
      ...
      c4strcpy( fullPathTagName, fullPathTagNameIn ) ;
      ...
      FILE4 groupFile ;
      if ( file4open( &groupFile, index->data->codeBase, groupFileNameWithPath, 0 ) )  // cannot access file, just return
         return ;
      ...
      if ( file4lenLow( &groupFile ) == 0 )
         groupFile.isTemp = 1 ;  // has the effect of removing the file
