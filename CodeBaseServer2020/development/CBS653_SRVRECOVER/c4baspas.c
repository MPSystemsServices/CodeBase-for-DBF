...
#include "d4all.h"
   ...
   short S4FUNCTION code4unlockAutoSetCB( CODE4 *cb, short value )
   {
      #ifdef E4VBASIC
         if ( c4parm_check( cb, 1, E40133 ) ) return -1 ;
      #endif
   ...
   */
   ...
   long S4FUNCTION RepWndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
   {
   ...
   void report4registerClass( REPORT4 *r4 )
   {
         ...
         if( GetClassInfo( (HINSTANCE)r4->hWnd, "RepClass", &temp ) )
            UnregisterClass( "RepClass", (HINSTANCE)r4->hWnd ) ;
         ...
         wndclass.lpszClassName = "RepClass" ;
   ...
   short S4FUNCTION report4doCB( REPORT4 *r4 )
   {
         ...
         hWnd = CreateWindow( "RepClass", "RepWindow", WS_OVERLAPPED,
      ...
      /*      r4->codeBase->hInst, (void FAR*) NULL); */
         ...
         */
         UnregisterClass( "RepClass", (HINSTANCE)hWnd ) ;
      ...
      if ( (len = expr4vary( e4, &result ) ) > 0 )
      {
         ...
         switch ( e_type )
         {
            ...
            case r4num_doub:
               ...
               #else
                  memcpy(temp, ecvt( *(double *)result, strlen(result),&dec, &sign), strlen(temp)) ;
               #endif
               break ;
