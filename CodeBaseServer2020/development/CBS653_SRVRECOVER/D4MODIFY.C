...
/* override internal S4CLIENT -- not implemented (yet) */
...
// AS 01/31/01 - Not available for now.  too complex
//
//
// /* S4CLIENT */
...
static int d4copy( DATA4 *d1, DATA4 *d2, short nFieldsMatch )
{
   ...
   while ( !d4eof( d2 ) && ( error4code( c4 ) == 0 ) )
   {
      ...
      if ( nFieldsMatch == -1 || (areMemosOrNulls == 1) )
      {
         ...
         c4memcpy( d1->record, d2->record, d4recWidth( d2 ) ) ;
      d4append( d1 ) ;
      if ( d4skip( d2, 1L ) != 0 )
         break ;
   }
...
static int createAndCloseEmptyFile( CODE4 *c4, const char *nameOfFileInSamePath, char *outFileName )
{
   FILE4 newFile ;

   int rc = createReplacementFile( c4, &newFile, nameOfFileInSamePath ) ;
   if ( rc != 0 )
      return rc ;
...
static DATA4 *d4fieldsAddCreate( DATA4 *d4, char *fileName, FIELD4INFO *fieldInfo )
{
   ...
   c4->safety = 0 ;
      ...
      c4->accessMode = d4->dataFile->file.lowAccessMode ;
   #endif
...
DATA4 *S4FUNCTION d4fieldsAdd( DATA4 *d4, short nFields, FIELD4INFO *fieldsToAdd )
{
   ...
   char buf[258] ;
   if ( createAndCloseEmptyFile( c4, d4->dataFile->file.name, buf ) != 0 )
      return 0 ;
