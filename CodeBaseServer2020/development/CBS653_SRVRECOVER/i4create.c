...
#include "d4all.h"
   ...
   int S4FUNCTION i4addEntryToGroupFile( INDEX4 *i4, const char *tagName )
   {
      // used to add a single new tag entry to a group file...- just appends...
         ...
         if ( file4open( &i4->file, c4, buf, 0 ) != 0 )  // can't open group file - major failure...
   ...
   INDEX4 *S4FUNCTION i4create( DATA4 *d4, const char *fileName, const TAG4INFO *tagData )
   {
      ...
      INDEX4 *index = i4createLow( d4, fileName, tagData ) ;
      ...
      /* !S4CLIENT */
         ...
         static int i4createIndexStructures( DATA4 *d4, INDEX4 **i4, INDEX4FILE **indexFile )
         {
         ...
         #ifdef S4STAND_ALONE
            static int i4setFileName( INDEX4 *i4, DATA4 *d4, const char *fileName )
            {
               if ( fileName == 0 )
            ...
            static int i4setupIndexMemory( INDEX4FILE *indexFile, CODE4 *c4 )
            {
               indexFile->blockMemory = mem4create( c4, c4->memStartBlock,
                    (sizeof(B4BLOCK)) + c4->foxCreateIndexBlockSize - (sizeof(B4STD_HEADER)) - (sizeof(B4NODE_HEADER)), c4->memExpandBlock, 0 ) ;
            ...
            static int i4initTagIndex( TAG4FILE *tagIndex, DATA4 *d4, INDEX4FILE *indexFile, char *indexName )
            {
               ...
               if ( c4->foxCreateIndexBlockSize != B4BLOCK_SIZE_INTERNAL || c4->foxCreateIndexMultiplier != 1 )
               {
                  // requesting CodeBase specific create with modified block size and multiplier...
                  if ( ( c4->foxCreateIndexBlockSize % c4->foxCreateIndexMultiplier ) != 0 || c4->foxCreateIndexMultiplier > 1024 )  // there is a remainder - this is an error...- current max of multiplier is 1024
                     return error4( c4, e4index, 85305 ) ;
                  if ( (c4->foxCreateIndexBlockSize % B4BLOCK_SIZE_INTERNAL) != 0 || c4->foxCreateIndexBlockSize < B4BLOCK_SIZE_INTERNAL )  // must be multiple of 512
                     return error4( c4, e4index, 85305 ) ;
                  tagIndex->header.codeBaseNote = 0xABCD ;  // indicates CodeBase specific and to use values...
                  tagIndex->header.blockSize = c4->foxCreateIndexBlockSize ;
                  tagIndex->header.multiplier = c4->foxCreateIndexMultiplier ;
                  indexFile->blockSize = c4->foxCreateIndexBlockSize ;
                  indexFile->multiplier = c4->foxCreateIndexMultiplier ;
               }
         ...
         static INDEX4 *i4createLow( DATA4 *d4, const char *fileName, const TAG4INFO *tagData )
         {
            ...
            int rc = file4create( &indexFile->file, c4, buf, 1 ) ;
            if ( rc )
            {
               if ( rc > 0 )
                  error4set( c4, rc ) ;
         ...
         TAG4 *S4FUNCTION t4create( DATA4 *d4, const TAG4INFO *tagData, INDEX4 *i4ndx, int useTempTagFileNames )
         {
            ...
            if ( buf[0] != 0 )
            {
               if ( i4ndx != 0 && i4->path != 0)
               {
                  ...
                  u4nameCurrentExtended( buf, sizeof( buf ), tagData->name, i4ndx->path ) ;
               }
               else
               {
      ...
      /* LY 2001/09/13 : added S4LINUX */
      #if defined(S4WIN32) || defined(S4LINUX)  // CS 2001/04/12
         ...
         )
         {
            ...
            CODE4 *c4 = data->codeBase ;
               ...
               {
                  c4strcpy( tagNameBuf, tagInfo[0].name ) ;
            ...
            // AS 06/09/00 was not compiling in S4OFF_MULTI
               ...
               c4->accessMode = data->dataFile->file.lowAccessMode ;
            #endif
