...
static int dataList4expandFromDbTree( DATA4LIST *listIn, CODE4 *codeBase )
{
   ...
   for( i = listIn->pointersUsed-1; i >= 0; i-- )
   {
      ...
      {
         if ( dataList4add( listIn, codeBase, relateParent ) < 0 )
            return -1 ;
...
static int log4addToList( L4LOGICAL *log, E4INFO *infoPtr, DATA4LIST *listIn )
{
   ...
   if ( infoPtr->functionI <= E4LAST_FIELD )
      if ( dataList4add( listIn, log->codeBase, relate4lookupRelate( (RELATE4 *)&log->relation->relate, f4data(infoPtr->fieldPtr)) ) < 0 )
         return -1 ;
   ...
   {
      if ( log4addToList( log, infoPtr, listIn ) < 0 )
         return -1 ;
