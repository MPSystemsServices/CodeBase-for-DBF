...
#include "d4all.h"
   ...
   // AS 01/16/01 - not available for clipper
   #ifndef S4CLIPPER
      static void r4reindexStatusInit( REINDEX4STATUS *reindexStatus, INDEX4FILE *i4file )
      {
         reindexStatus->c4 = i4file->codeBase ;
         ...
         reindexStatus->c4->incrementMax = 2 * dfile4recCount( i4file->dataFile, 1 ) ;
   ...
   static void r4reindexStatusInitDone( REINDEX4STATUS *reindexStatus )
   {
   ...
   static void r4reindexStatusNextTag( REINDEX4STATUS *reindexStatus )
   {
      reindexStatus->tagOn++ ;
   ...
   static void r4reindexStatusFinalSet( REINDEX4STATUS *reindexStatus, double val )
   {
   ...
   static void r4reindexStatusInitUndo( REINDEX4STATUS *reindexStatus )
   {
   ...
   int S4FUNCTION i4reindex( INDEX4 *i4 )
   {
      ...
      #ifdef TIME4STATUS
         REINDEX4STATUS reindexStatus ;
         r4reindexStatusInit( &reindexStatus, indexFile ) ;
      #endif
      ...
      for( ;; )
      {
         ...
         #ifdef TIME4STATUS
            r4reindexStatusInitDone( &reindexStatus ) ;
         #endif
         ...
         for( tagOn = 0 ;; )
         {
            ...
            #ifdef TIME4STATUS
               r4reindexStatusNextTag( &reindexStatus  ) ;
            #endif
         }
         ...
         #ifdef TIME4STATUS
            r4reindexStatusFinalSet( &reindexStatus, .955 ) ;
         #endif
         ...
         if ( rc < 0 )
         ...
         #ifdef TIME4STATUS
            r4reindexStatusFinalSet( &reindexStatus, .975 ) ;
         #endif
            ...
            for( tagOn = 0 ; ; )
            {
                  ...
                  while ( b4nodeValid( goTo ) )
                  {
                     ...
                     if ( rc < 0 )
                        ...
                        if (block->header.nodeAttribute >= 2 ) /* if block is a leaf */
                        {
                           ...
                           long longVal = x4reverseLong( (void *)&block->nodeHdr.recNumMask[0] ) ;
                        ...
                        else /* if block is a branch */
                        {
                           ...
                           {
                              long longVal = x4reverseLong( (void *)swapPtr ) ;
               ...
               if ( rc < 0 )
                  break ;
               #ifdef TIME4STATUS
                  r4reindexStatusFinalLoop( &reindexStatus ) ;
               #endif
            }
            if ( rc < 0 )
               break ;
      ...
      #ifdef TIME4STATUS
         r4reindexStatusInitUndo( &reindexStatus ) ;
      #endif
   ...
   int r4reindexTagHeadersWrite( R4REINDEX *r4 )
   {
      ...
      for ( TAG4FILE *tagOn = 0 ;; )
      {
         ...
         if ( i4file->tagIndex->header.typeCode >= 64 )
            file4longAssign( fileHeaderPos, ((S4UNSIGNED_LONG)iTag) * (B4BLOCK_SIZE_INTERNAL), 0 ) ;
         else
            file4longAssign( fileHeaderPos, 0, 0 ) ;
   ...
   int r4reindexWriteKeys( R4REINDEX *r4, short int errUnique )
   {
      ...
      int isUnique = t4->header.typeCode & 0x01 ;
      int isFirst = 1 ;
