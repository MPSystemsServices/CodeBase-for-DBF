...
#include "d4all.h"
   ...
   int tfile4stok( TAG4FILE *t4, char *buf, const char *str, int len )
   {
      ...
      */
      ...
      int lenOut ;
      (*t4->stok)( collation4get( t4->collateName ), ptr, str, len, &lenOut ) ;
      ...
      if ( d4recCountLessEq( data, tfile4recNo( tag ) ) == 0 )  /* past eof */
         while ( d4recCountLessEq( data, tfile4recNo( tag ) ) == 0 )  /* past eof */
         {
            ...
            if ( dfile4lockTestFile( data->dataFile, data4clientId( data ), data4serverId( data ), lock4write ) == 1
               || dfile4lockTestFile( data->dataFile, data4clientId( data ), data4serverId( data ), lock4read ) == 1 )
         #else
   ...
   int S4FUNCTION d4seek( DATA4 *data, const char *str )
   {
      ...
      short S4FUNCTION d4seekNextN( DATA4 *data, const char *str, const short lenIn )
      {
         ...
         int rc ;
            ...
            c4->errGo = 0 ;
            rc = d4seekServer( data, str, lenIn, 1, CON4SEEK ) ;
            c4->errGo = oldErrGo ;
            if ( rc < 0 && error4code( c4 ) == 0 )
               return error4( c4, rc, E92908 ) ;
            return rc ;
   ...
   short S4FUNCTION d4seekN( DATA4 *data, const char *inputKey, const short inputKeyLen )
   {
      ...
      #ifdef S4CLIENT
         return d4seekServer( data, inputKey, inputKeyLen, 0, CON4SEEK ) ;
      #else
         c4 = data->codeBase ;
         if ( c4 == 0 )
            return e4info ;
         ...
         #endif
         {  /* for S4FOX */
            /* BCR 09/14/00 -- tfile4stok conversion is needed for LINUX too.
            #ifdef S4UNIX
               if ( data4seekStrConvertKey( tfile, inputKey, outputKey, inputKeyLen, &outputKeyLen ) < 0 )
                  return -1 ;
            #else */
               #ifdef S4CLIPPER
                  if ( data4seekStrConvertKey( tfile, inputKey, outputKey, inputKeyLen, &outputKeyLen ) < 0 )
