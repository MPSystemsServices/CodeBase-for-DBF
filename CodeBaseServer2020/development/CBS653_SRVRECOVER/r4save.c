...
DATA4 S4PTR* S4FUNCTION relate4lookup_data( RELATE4 *relate, char *dname )
{
   ...
   while( relate_on != NULL )
      {
      if( stricmp( dname, d4alias(relate_on->data) ) == 0 )
                                  ...
                                  char *buf, char *name_buf )
{
   ...
   report->report_file_name = (char *)u4allocEr( report->codeBase, strlen(name_buf) + 1 );
   if( !report->report_file_name )
      return -1;
   strcpy( report->report_file_name, name_buf );
