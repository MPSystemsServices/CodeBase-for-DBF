...
#include "d4all.h"
   ...
   // AS Aug 1/03 - Large file support
   static FILE4LONG compress4handlerOffsetGet( COMPRESS4HANDLER *compress, long blockIndex )
   {
      if ( compress->isLong == 1 )
      {
         assert5( compress->longCompress != 0 ) ;
         assert5( blockIndex <= compress->longCompress->numOffsets ) ;
         return compress->longCompress->offsets[blockIndex] ;
      }
      else
      {
         assert5( compress->shortCompress != 0 ) ;
         assert5( blockIndex <= compress->shortCompress->numOffsets ) ;
         FILE4LONG len ;
...
static unsigned file4readLowPhysical( FILE4 *f4, FILE4LONG pos, void *ptr, unsigned len )
{
   ...
   if ( file4longError( pos ) == (unsigned long)-1L )
   {
      ...
      // AS Feb 17/06 - for Windows CE, it is possible to get errors if the card is not ready...retry if access denied error
      // Microsoft knowledgebase article #811693
      BOOL blRes;  // CS 2007/10/03 Declare outside of "for" loop.
      #ifdef S4WINCE
         for ( numTries = 0 ;; numTries++ )
         {
      #endif
            // MessageBox( 0, L"toread", L"DEBUG", MB_OK | MB_ICONSTOP ) ;
            blRes = ReadFile( (HANDLE)f4->hand, ptr, len, &urc, 0 ) ;
...
// AS Oct 7/03 - improve performance without compression
   ...
   static unsigned file4readDecompress( FILE4 *f4, FILE4LONG pos, void *ptr, unsigned len )
   {
      ...
      unsigned long blockOffset = file4longMod( blockOffsetLong, f4->actualBlockSize ) ;
      ...
      while ( lenLeft > 0 )
      {
         ...
         // AS Aug 1/03 - Large file support
         FILE4LONG compressOffset = compress4handlerOffsetGet( f4->compressInfo, blockOn ) ;
         if ( file4longEqualLong( f4->uBufFilePos, compressOffset ) )
         {
            ...
            if ( blockOn + 1 == compress4handlerNumOffsetsGet( f4->compressInfo ) )
               doneRead = 1 ;
         }
         else
         {
            ...
            {
               lenToReadLong = compress4handlerOffsetGet( f4->compressInfo, blockOn+1 ) ;
               file4longSubtractLong( &lenToReadLong, &compressOffset ) ;
            ...
            if ( file4longLess( maxLen, amountAvailInBlock ) )
            {
               // not necessarily the case.
               assert5( blockOn == compress4handlerNumOffsetsGet( f4->compressInfo ) - 1 ) ;  // ensure we are on the last block in this case
               // long svLen = outLen ;
            ...
            f4->uBufFilePos = compress4handlerOffsetGet( f4->compressInfo, blockOn ) ;
         ...
         else
         {
            ...
            if ( file4longGreater( tempVal, lenLeft ) )
               lenToCopy = lenLeft ;
            else
            {
...
unsigned file4readInternal( FILE4 *f4, FILE4LONG pos, void *ptr, unsigned len )
{
      ...
      #else
         if ( f4->hand == INVALID4HANDLE && ( f4->fileCreated == 1 || f4->isTemporary == 0 )  )
      #endif
      {
      ...
      else
      {
         ...
         for ( vPtr = &valueStr[1]; vPtr <= &valueStr[numdigitsTotal+1]; vPtr++ )
         {
            ...
            else
            {
               ...
               tempDbl = (double) j ;
               *vPtr = j + '0' ;
               // AS Mar 22/07 - this fix doesn't work for f4fcvt(0.83, 8,...)  -- if someone has the example where it is required perhaps that will solve
   ...
   void file4compressInitUndo( FILE4 *file )
   {
      ...
      if ( file->compressInfo != 0 )
      {
         if ( file->freeCompressInfo == 1 )
         {
            ...
            if ( file->compressInfo->shortCompress != 0 )
            {
               assert5( file->compressInfo->isLong == 0 ) ;
               u4free( file->compressInfo->shortCompress ) ;
            }
            if ( file->compressInfo->longCompress != 0 )
            {
               assert5( file->compressInfo->isLong == 1 ) ;
               u4free( file->compressInfo->longCompress ) ;
            }
