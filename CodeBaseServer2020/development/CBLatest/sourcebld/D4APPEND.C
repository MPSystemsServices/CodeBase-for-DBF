...
short S4FUNCTION d4appendStartLow( DATA4 *data, short useMemoEntries )
{
   ...
   assert5( c4 != 0 ) ;
      ...
      #endif
   #else
      int rc = d4updateRecord( data, 0, 1 ) ;   /* returns -1 if error4code( c4 ) < 0 */
      if ( rc )
         return rc ;
...
short S4FUNCTION d4appendStart( DATA4 *data, short useMemoEntries )
{
   ...
   int rc = d4appendStartLow( data, useMemoEntries ) ;
      ...
      if ( d4lockTestFile( data ) != 1 )  // AS Nov 25, 2013.  actually we don't need to unlock the append bytes if they are already locked...
      {

         int rc2 = d4unlockFile( data, -1 ) ;
         if ( rc2 == 0 )
            rc2 = d4unlockRecords( data, -1 ) ;
         if ( rc == 0 )
            rc = rc2 ;
      }
   #endif
