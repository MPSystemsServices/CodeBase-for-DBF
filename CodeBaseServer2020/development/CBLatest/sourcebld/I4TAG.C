      ...
      int S4FUNCTION tfile4seek( TAG4FILE *t4, const void *ptr, const int lenIn )
      {
         ...
         int tfile4leafSplit( TAG4FILE *t4, B4BLOCK *oldBlock, B4BLOCK *newBlock )
         {
            ...
            if ( b4numKeys( oldBlock ) == 1 )
            {
               ...
               {
                  // AS Jan 20/15 - in this case, we are appending at the very end of the index and the key was not found.
                  // What we should do is: the old block is ok as is, we want to put the new key into the new block
