...
HANDLE hEventLog = RegisterEventSource(0, "event.c");
char p[256] = "Event description";
char *pp[] = {p};
sprintf(p, "server4taskbarIcon %s", hWndGlobal==0?"bad":"good");
...
DeregisterEventSource(hEventLog);
         ...
         BOOL rc;
         rc = Shell_NotifyIcon( operation, &iconInfo ) ;
         if (rc == FALSE)
         {
            Sleep(20000);
            rc = Shell_NotifyIcon( operation, &iconInfo ) ;
