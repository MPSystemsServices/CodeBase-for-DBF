...
Contact Sequiter Software, Inc. for distribution information." ;
   ...
   void S4FUNCTION setService( void )
   {
      ...
      void initAboutDialog( HWND hWnd, LPARAM lParam )
      {
         ...
         wsprintf( buffer, "%s %d", tmpStr, server4numClients( serverPtr ) ) ;
         ...
         wsprintf( buffer, "%s %d", tmpStr3, server4maxSockets( serverPtr ) ) ;
         ...
         wsprintf( buffer, "%s %d", tmpStr3, server4portNumber( serverPtr ) ) ;
      ...
      static void initAboutLicense( HWND hWnd, LPARAM lParam )
      {
         ...
         short security = server4securityOption() ;
         SetWindowLong( hWnd, DWL_USER, (long)(HMENU)lParam ) ;

         // CS 2002/06/19 display build number in About box
         // 124 Build number
         if ( CustomerStamp.BuildNumber == 0 )
            buffer[0] = 0 ;
         else
            sprintf( buffer, "Build %ld", CustomerStamp.BuildNumber ) ;
         ...
         if ( security >= 2 )
            wsprintf( buffer, "%s  %s", tmpStr, server4CustomerFromStamp() ) ;
         else
         ...
         if ( security >= 2 )
            wsprintf( buffer, "%s Distribution Number:  %s", tmpStr, server4SerialFromStamp() ) ;
         else
         ...
         switch( message )
         {
            ...
            case WM_QUERYENDSESSION:
               ...
               else
               {
                  ...
                  wsprintf( buffer+strlen( buffer ), "%d", server4numClients( serverPtr ) ) ;
         ...
         if ( !hPrevInstance )
         {
            ...
            hInst = hInstance ;
               ...
               else
               {
                  ...
                  if ( !CloseHandle( serverThread ) )
                     error4( 0, e4info, E70227 ) ;
               }
            #else
               d4server( lpCmdLine ) ;
            #endif
   ...
   #endif
   {
      ...
      static int server4securityCheck( CODE4 *c4 )
      {
          ...
          strcpy( Cname, server4CustomerFromStamp() ) ;
          strcpy( Serial, server4SerialFromStamp() ) ;
          #ifdef S4SERVER_GUI
             SetWindowText( hRegistered, Cname ) ;
             SetWindowText( hSerialNo, Serial ) ;
             time( &startTime ) ;
      ...
      #endif
      {
         ...
         for ( ;; )
         {
            ...
            if ( rc != 0 )
            {
               ...
               #ifdef S4OLEDEBUG_PRINT
                  log5( "d4server() returned from server4waitOnQuitOrExit()\r\n" ) ;
               #endif
               ...
               for( rc = 0 ; rc >= 0 ; )
               {
                  ...
                  if ( server4quit( serverPtr, -1 ) > 0 )
                  {
                     ...
                     {
                        if ( server4numClients( serverPtr ) == 0 )   /* means r4quit */
                        {
      ...
      int server4initUser( Bool5 prompt )
      {
         ...
         int rc = r4success, temp ;
            ...
            /* AS 11/10/98 -- modified so that if server->systempPath is not a fully quantified
               ...
               u4nameMakeFindDrive( configBuff, sizeof( configBuff ), server4systemPath( serverPtr ), "CONFIG4.dbf" ) ;
            */
