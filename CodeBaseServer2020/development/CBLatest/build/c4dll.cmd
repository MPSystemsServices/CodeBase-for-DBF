...
if not exist "%INTERMEDIATE%" (
   echo ERROR: Intermediate folder "%INTERMEDIATE%" not found.
   exit /b 1
)

if not exist "%OUTPUT%" (
   echo ERROR: Output folder "%OUTPUT%" not found.
   exit /b 1
)
...
REM *** compile
call c4dll_compile %CONTINUE% %DEBUG% /O "%INTERMEDIATE%" /S "%SOURCE%" %COMPOPTS% @%listfile%
echo %s4echo%
...
call c4dll_link %DEBUG% /I "%INTERMEDIATE%" /N "%DLLNAME%" /O "%OUTPUT%" %LINKOPTS% @%listfile%
echo %s4echo%
