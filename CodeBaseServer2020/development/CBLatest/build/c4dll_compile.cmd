...
if "%SOURCE%" NEQ "%OUTPUT%" (
   ...
   if exist "%SOURCE%\c4dll.res" (
      xcopy /r /y "%SOURCE%\c4dll.res" "%OUTPUT%\." > nul
   )
   if exist "%SOURCE%\c4dll32.def" (
      xcopy /r /y "%SOURCE%\c4dll32.def" "%OUTPUT%\." > nul
   )
   if exist "%SOURCE%\c4dll_lib.lst" (
      xcopy /r /y "%SOURCE%\c4dll_lib.lst" "%OUTPUT%\." > nul
   )
   if exist "%SOURCE%\zlib.lib" (
      xcopy /r /y "%SOURCE%\zlib.lib" "%OUTPUT%\." > nul
   )
)
...
:CompileOne
   ...
   @echo on
   cl -DS4DLL_BUILD -DWIN32 -c -nologo -MT -W1 -Zp1 %DEBOPTS% %COMPOPTS% %PCH% -Tp "%SOURCE%\%~1" >> OUT
   @echo off
   set PCH=-Yud4all.h
