      ...
      int S4FUNCTION i4closeLow( INDEX4 *i4 )
      {
         ...
         for( ;; )
         {
            ...
            if ( tagOn->tagFile->expr != 0 ) /* could be NULL if improper open or if unsupported expression (read-only) */
               ...
               {
                  expr4contextClear( tagOn->tagFile->expr, i4->data ) ;  // AS May 7, 2014 clears the expr4 context if necessary to avoid bad pointers and so forth when a clone is closed
